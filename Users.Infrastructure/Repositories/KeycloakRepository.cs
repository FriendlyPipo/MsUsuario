...
namespace Users.Infrastructure.Repositories
{
    public class KeycloakRepository : IKeycloakRepository
    {
        ...
        {
            var url = $"{_configuration["Keycloak:BaseUrl"]}/realms/GoTicket/protocol/openid-connect/token";
            var clientId = _configuration.GetValue<string>("Keycloak:ClientId") ?? throw new KeycloakException("Configuration 'Keycloak:ClientId' is missing.");
            var clientSecret = _configuration.GetValue<string>("Keycloak:ClientSecret") ?? throw new KeycloakException("Configuration 'Keycloak:ClientSecret' is missing.");
        ...
        public async Task AssignTypeToUserAsync(string keycloakUserId, UserType userType, string token)
        {
            ...
            if (!response.IsSuccessStatusCode)
            {
                throw new KeycloakException($"Failed to assign role '{userType.ToString()}' to user with ID '{keycloakUserId}' in Keycloak. Status code: {response.StatusCode}");
            }
        }
